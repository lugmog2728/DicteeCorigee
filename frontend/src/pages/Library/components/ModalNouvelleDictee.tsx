...
export default function ModalNouvelleDictee({ onClose, onSubmit }: ModalNouvelleDicteeProps) {
  ...
  const [temps, setTemps] = useState<DicteeCreate['temps'] | ''>('')
  ...
  const fieldClass = 'bg-[#f3f3f5] border border-[rgba(0,0,0,0)] rounded-[8px] px-3 text-[14px] text-[#717182] placeholder:text-[#717182] outline-none focus:border-(--ocean-blue-500) w-full'
  const labelClass = 'text-[14px] font-medium text-[#0a0a0a] leading-[14px] mb-2 block'

  async function handleSubmit() {
    if (!niveau || !periode || !temps) return
    await onSubmit?.({
      titre,
      niveau: niveau as DicteeCreate['niveau'],
      periode: periode as DicteeCreate['periode'],
      temps: temps as DicteeCreate['temps'],
      tag: tag || undefined,
      texte,
      errors: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div className="relative bg-white border border-[rgba(0,0,0,0.1)] rounded-t-[20px] sm:rounded-[10px] shadow-[0px_10px_15px_-3px_rgba(0,0,0,0.1),0px_4px_6px_-4px_rgba(0,0,0,0.1)] w-full sm:w-[583px] max-h-[92dvh] sm:max-h-[90vh] overflow-y-auto p-4">
        {/* Header */}
